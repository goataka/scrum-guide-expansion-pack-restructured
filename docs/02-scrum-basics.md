...
> [!NOTE]
> **未知が既知より多く**、**原因と結果は後から明確**になる。**複雑性思考**がツールと洞察を提供し、**創発**を通じて機会を捉え**透明性・検査・適応**で改善を実現。
...
> [!NOTE]
> **複雑なシステム内の相互作用**から予測できないパターンが生じる現象。**タイムボックスや役割**などの制約が創発を導き、**望ましいパターンを発見し増幅**する。
...
> [!NOTE]
> **軌道確認・アクション・作業方法決定・対立解決・問題修正**を自ら行う。マネージャーは**指示ではなくリーダーシップ**を発揮し、**価値中心の自己管理チーム**が創発に不可欠。
...
> [!NOTE]
> **優秀性を追求**し**尊敬・透明性・説明責任**で協働。**ライフサイクル全体に責任**を負い、**技術的卓越性**には**クリーンコード・TDD・継続的デリバリー**などが含まれる。
...
> [!NOTE]
> **5つの柱**で構成。**公開**は信頼を必要とし帯域幅の太い会話で学習。**勇気**は困難に立ち向かう力。**確約**はゴール達成と価値実現への約束。**集中**はゴールへの進捗。**尊敬**はアイデアを批評し人を批判しない。
...
> [!NOTE]
> **OODAループ**（**観察・状況判断・意思決定・行動**）の視点から理解可能。スクラムの価値基準が**すべてに適用**され、**創発的ソリューション**を育む。
