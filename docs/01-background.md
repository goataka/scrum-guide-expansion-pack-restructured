...
> **要約**: スクラムフレームワークはKen SchwaberとJeff Sutherlandによって開発され、2020年版スクラムガイドがその本質を説明している。広く採用されるため、ガイドはシンプルに保たれており、複数のバージョンや解説書が存在する。
...
> **要約**: スクラムは主体性、透明性、継続的改善などの価値観とともに適用すべき。提示されたガイダンスを超越し、持続的な好奇心を持って探求し続けることが期待される。
...
> **要約**: この拡張パックは自己管理チームによる製品発見から価値実現までのすべての側面を支援する。元々はソフトウェア開発から始まったが、現在は多様な専門分野で広く採用され、複雑な仕事における価値提供を可能にしている。
...
> **要約**: ステークホルダー価値は、ステークホルダーが重要と考えるニーズを指すが、彼ら自身が何が価値あるかを認識しているとは限らない。観察やエビデンスを通じて価値が顕在化し、優先順位に影響を与える。価値はエビデンスで確認されるまで仮説である。
...
> **要約**: スクラムは複雑なプロダクトデリバリーのためのフレームワークで、専門性を超えた能力が必要とされる。プロダクトライフサイクル全体をカバーし、変化への適応と価値創造を支援する。因果関係は振り返りで明らかになる。
...
> **要約**: スクラムはステークホルダーのニーズに一貫して対応する環境を育み、反復的・漸進的アプローチによりリスクを削減する。問題探求から価値検証までのバランスを取ることを支援する。
...
> **要約**: リスクは将来の不利な結果をもたらす要因であり、時間経過でも予測不可能なため事前の備えが重要。市場リスク、フィット、技術、コンプライアンスなど多様な要因が含まれ、スクラムはこれらの管理と機会発見を支援する。
...
> **要約**: スクラムは問題提示者と解決者の距離を縮める。サポーターがスクラム採用を支援し、プロダクトオーナーがゴールを設定、チームが価値あるアウトカムを創出し、全員で検査・適応を繰り返すサイクルで機能する。
...
> **要約**: リリースはプロダクトの新バージョンをステークホルダーが利用可能にするプロセスであり、開発サイクルの転換点となる。開発から使用可能状態への移行を示し、ステークホルダー価値の実現可能性を具現化する。
...
> **要約**: スクラムは意図的に不完全であり、詳細なプロセスではなく関係性と相互作用を導くフレームワークを提供する。様々な手法で補完可能だが、その適用は文脈により異なる。
...
> **要約**: スクラムは既存プラクティスと統合するか、場合によっては置き換える。チーム、サポーター、マネジメント、環境、技術の効果性を透明に評価し、継続的改善を可能にする。
...
> **要約**: スクラムという用語は竹内弘高と野中郁次郎によりラグビーから借用され、知識を企業全体に急速に広めて優れたプロダクトを提供するチームを説明するために考案された。
